...
def test_overview_urlaub_shows_blk_with_cell_blocker(client, session: Session):
    """URLAUB assignment renders as 'BLK' with class cell-blocker."""
    ids = _setup_overview(session)
    session.add(Assignment(
        trainee_id=ids["trainee_id"], schoolyear_id=SY, kw=40, jahr=2025,
    ...
    assert ">BLK<" in r.text


def test_overview_frei_shows_blk_with_cell_blocker(client, session: Session):
    """FREI assignment renders as 'BLK' with class cell-blocker."""
    ids = _setup_overview(session)
    session.add(Assignment(
        trainee_id=ids["trainee_id"], schoolyear_id=SY, kw=40, jahr=2025,
